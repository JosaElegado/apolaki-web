...
for _w in (400, 500, 600, 700):
    css = css.replace("url('../fonts/apolaki-sans-%d.woff2')" % _w,
                      "url(%s)" % b64('assets/fonts/apolaki-sans-%d.woff2' % _w, 'font/woff2'))
css = css.replace("url('../fonts/apolaki-serif-400.woff2')",
                  "url(%s)" % b64('assets/fonts/apolaki-serif-400.woff2', 'font/woff2'))
css += """
/* ---- single-file router ---- */
...
PAGES = [
    ('home',        'index.html',              None),   # the logo is the home link
...
nav_links = "".join(
    ...
    for key, _, label in PAGES if label)
mob_links = "".join(
...
function route(){
  ...
  var T={home:'Apolaki | Trusted Solar Installers Near You',homeowners:'Solar for Your Home | Apolaki',
    ...
    privacy:'Privacy Policy | Apolaki',terms:'Terms of Use | Apolaki'};
